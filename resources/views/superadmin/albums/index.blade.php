 ...
 <div class="px-8 pb-8 flex-1 w-full max-w-[1400px]">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mb-12">
        @forelse($albums as $album)
            <div x-data="{ visible: {{ $album->is_visible ? 'true' : 'false' }} }" class="group rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border flex flex-col" :class="$store.theme.darkMode ? 'bg-[#111827] border-gray-800' : 'bg-white border-gray-100'">
                <!-- Album Thumbnail -->
                <div class="h-56 relative overflow-hidden">
                    @if($album->photos->isNotEmpty())
                        <img src="{{ asset('storage/' . $album->photos->first()->path) }}" alt="{{ $album->title }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
                    @else
                    ...
                    <div class="absolute top-4 left-4">
                        <span class="px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm bg-blue-100 text-blue-700">{{ $album->category }}</span>
                    </div>

                    <!-- Visibility Toggle -->
                    <div class="absolute top-4 right-4">
                        <button @click="visible = !visible; fetch('{{ route('superadmin.albums.toggle', $album) }}', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } })"
                ...
                <!-- Card Content -->
                <div class="p-6">
                    <h3 class="text-lg font-extrabold mb-4 transition-colors duration-300 line-clamp-1" :class="$store.theme.darkMode ? 'text-white' : 'text-gray-900'">{{ $album->title }}</h3>
                    ...
                    <div class="flex items-center justify-between text-gray-400 mb-6 font-semibold">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs">{{ \Carbon\Carbon::parse($album->event_date)->translatedFormat('d F Y') }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs">{{ $album->photos->count() }} Foto</span>
                        </div>
                    </div>

                    <div class="flex items-center gap-2">
                        <a :href="visible ? '{{ route('album.show', $album) }}' : '{{ route('superadmin.albums.show', $album) }}'" 
                        ...
                        <a href="{{ route('superadmin.albums.edit', $album) }}" class="flex-1 py-2 rounded-xl bg-blue-50 text-blue-600 text-center text-xs font-black hover:bg-blue-100 transition-colors" :class="$store.theme.darkMode ? 'bg-blue-500/10 text-blue-400 hover:bg-blue-500/20' : ''">EDIT</a>
                        <form id="delete-form-{{ $album->id }}" action="{{ route('superadmin.albums.destroy', $album) }}" method="POST" class="flex">
                            @csrf @method('DELETE')
                            <button type="button" @click="confirmDelete({{ $album->id }})" class="w-8 h-8 rounded-lg flex items-center justify-center text-red-500 hover:bg-red-50 transition-colors">
