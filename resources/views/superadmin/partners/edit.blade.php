...
<div class="px-8 pb-8 flex-1 w-full" :class="$store.theme.darkMode ? '' : 'bg-[#F3F4F6]'">
    ...
    <form id="partnerForm" action="{{ route('superadmin.partners.update', $partner->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        @csrf
        @method('PUT')
            ...
            <div class="rounded-3xl p-8 shadow-xl transition-all duration-200" :class="$store.theme.darkMode ? 'bg-[#111827]/50' : 'bg-white hover:shadow-2xl hover:-translate-y-1'">
                ...
                <div class="space-y-3">
                    ...
                    <textarea name="description" id="description" rows="5"
                              ...
                              placeholder="Berikan deskripsi singkat mengenai kerja sama atau profil mitra...">{{ old('description', $partner->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
