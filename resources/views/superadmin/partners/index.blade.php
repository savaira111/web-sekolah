...
@section('title', 'Mitra Industri - Admin Mahput')
...
@section('content')
<div class="px-8 pb-8 flex-1 w-full overflow-x-auto" x-data="{ isDeleteModalOpen: false, partnerToDelete: null }">
    ...
    <div class="flex items-center gap-2 mt-8 mb-6 text-[13px] font-medium transition-colors duration-300" :class="$store.theme.darkMode ? 'text-gray-400' : 'text-gray-500'">
        ...
        <span class="font-bold" :class="$store.theme.darkMode ? 'text-blue-400' : 'text-blue-600'">Manajemen Mitra</span>
    </div>
        ...
        <div class="flex flex-col sm:flex-row items-center gap-4">
            ...
            <div class="flex items-center p-1.5 rounded-full border transition-colors duration-300 shadow-sm" :class="$store.theme.darkMode ? 'bg-[#111827] border-gray-700' : 'bg-white border-gray-200'">
                <a href="{{ request()->fullUrlWithQuery(['filter' => 'all', 'page' => null]) }}"
                   ...
                   :class="$store.theme.darkMode ? '{{ $filter === 'all' ? 'bg-black/20 text-white shadow' : 'text-gray-400 hover:text-white' }}' : '{{ $filter === 'all' ? 'bg-gray-900 text-white shadow' : 'text-gray-600 hover:text-gray-900' }}'">Semua</a>
                <a href="{{ request()->fullUrlWithQuery(['filter' => 'active', 'page' => null]) }}"
                   class="px-5 py-2 rounded-full text-sm font-bold transition-all"
                   :class="$store.theme.darkMode ? '{{ $filter === 'active' ? 'bg-black/20 text-white shadow' : 'text-gray-400 hover:text-white' }}' : '{{ $filter === 'active' ? 'bg-gray-900 text-white shadow' : 'text-gray-600 hover:text-gray-900' }}'">Aktif</a>
                <a href="{{ request()->fullUrlWithQuery(['filter' => 'inactive', 'page' => null]) }}"
                   class="px-5 py-2 rounded-full text-sm font-bold transition-all"
                   :class="$store.theme.darkMode ? '{{ $filter === 'inactive' ? 'bg-black/20 text-white shadow' : 'text-gray-400 hover:text-white' }}' : '{{ $filter === 'inactive' ? 'bg-gray-900 text-white shadow' : 'text-gray-600 hover:text-gray-900' }}'">Nonaktif</a>
            </div>

            <a href="{{ route('superadmin.partners.create') }}" class="px-6 py-2.5 bg-[#3B82F6] hover:bg-[#2563EB] text-white text-sm font-bold rounded-full transition-all duration-300 shadow-lg shadow-blue-500/30 flex items-center justify-center gap-2 shrink-0">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div class="rounded-[2rem] p-6 transition-all duration-300 flex items-center justify-center gap-5 shadow-sm hover:-translate-y-1 hover:shadow-xl" :class="$store.theme.darkMode ? 'bg-[#111827] shadow-black/20' : 'bg-white shadow-gray-200/50'">
            ...
            <div class="flex flex-col">
                ...
                <h3 class="text-3xl font-black leading-none transition-colors duration-300 tracking-tight" :class="$store.theme.darkMode ? 'text-white' : 'text-[#111827]'">{{ number_format($total ?? 0) }}</h3>
            </div>
        </div>
        ...
        <div class="rounded-[2rem] p-6 transition-all duration-300 flex items-center justify-center gap-5 shadow-sm hover:-translate-y-1 hover:shadow-xl" :class="$store.theme.darkMode ? 'bg-[#111827] shadow-black/20' : 'bg-white shadow-gray-200/50'">
            ...
            <div class="flex flex-col">
                ...
                <h3 class="text-3xl font-black leading-none transition-colors duration-300 tracking-tight" :class="$store.theme.darkMode ? 'text-white' : 'text-[#111827]'">{{ number_format($active ?? 0) }}</h3>
            </div>
        </div>
        ...
        <div class="rounded-[2rem] p-6 transition-all duration-300 flex items-center justify-center gap-5 shadow-sm hover:-translate-y-1 hover:shadow-xl" :class="$store.theme.darkMode ? 'bg-[#111827] shadow-black/20' : 'bg-white shadow-gray-200/50'">
            ...
            <div class="flex flex-col">
                ...
                <h3 class="text-3xl font-black leading-none transition-colors duration-300 tracking-tight" :class="$store.theme.darkMode ? 'text-white' : 'text-[#111827]'">{{ number_format($inactive ?? 0) }}</h3>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-[2rem] p-6 lg:p-8 transition-colors duration-300 shadow-sm" :class="$store.theme.darkMode ? 'bg-[#111827] shadow-black/20' : 'bg-white shadow-gray-200/50'">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 pb-6 border-b transition-colors duration-300 gap-4" :class="$store.theme.darkMode ? 'border-gray-800' : 'border-gray-100'">
            <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <div class="flex items-center gap-3" x-data="{ catOpen: false }">
                    {{-- Pencarian --}}
                    <form action="{{ route('superadmin.partners.index') }}" method="GET" class="relative group">
                    ...
                    <div class="relative">
                        ...
                        <div x-show="catOpen" @click.outside="catOpen = false" x-cloak
                            ...
                            <a href="{{ request()->fullUrlWithQuery(['category' => null, 'page' => null]) }}"
                               class="block px-4 py-2.5 text-[13px] font-semibold transition-colors {{ !$category ? 'text-blue-600 bg-blue-50' : 'text-gray-700 hover:bg-gray-50' }}">Semua Tipe</a>
            ...
            <p class="text-[13px] font-bold transition-colors duration-300" :class="$store.theme.darkMode ? 'text-gray-400' : 'text-gray-400'">Menampilkan {{ $partners->firstItem() ?? 0 }}-{{ $partners->lastItem() ?? 0 }} dari {{ number_format($partners->total() ?? 0) }} mitra</p>
        </div>
                    ...
                    <tr class="border-b last:border-0 transition-colors group" :class="$store.theme.darkMode ? 'border-gray-800 hover:bg-black/20/50' : 'border-gray-50 hover:bg-gray-50/50'">
                        <td class="py-5 transition-colors duration-300 pl-2">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="w-14 h-14 rounded-xl overflow-hidden shrink-0 border-[3px] bg-white p-1" :class="$store.theme.darkMode ? 'border-slate-700/50' : 'border-gray-100'">
                                    <img src="{{ asset('storage/' . $partner->logo) }}" class="w-full h-full object-contain" alt="{{ $partner->name }}">
                                </div>
                                @else
                                ...
                                @endif
                                <div>
                                    <span class="font-bold text-[15.5px] block mb-0.5" :class="$store.theme.darkMode ? 'text-white' : 'text-[#111827]'">{{ $partner->name }}</span>
                                    <p class="text-[13px] transition-colors duration-300 font-medium truncate max-w-[250px]" :class="$store.theme.darkMode ? 'text-gray-400' : 'text-gray-500'">{{ Str::limit($partner->description, 50) }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-5">
                            <span class="inline-block px-3 py-1 font-black text-[10px] rounded-full uppercase tracking-widest transition-colors duration-300" :class="$store.theme.darkMode ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-50 text-indigo-600'">{{ $partner->category }}</span>
                        </td>
                        <td class="py-5">
                            @if($partner->website_url)
                            <a href="{{ $partner->website_url }}" target="_blank" class="text-[13px] font-semibold hover:text-blue-600 transition-colors flex items-center gap-1.5" :class="$store.theme.darkMode ? 'text-gray-300' : 'text-gray-600'">
                        ...
                        <td class="py-5 text-right pr-2">
                            <div class="flex items-center justify-end gap-2">
                                {{-- Status Toggle --}}
                                <button
                                    onclick="toggleStatus({{ $partner->id }}, {{ $partner->is_active ? 'true' : 'false' }}, '{{ route('superadmin.partners.toggle-status', $partner->id) }}', '{{ csrf_token() }}')"
                                    ...
                                    :class="$store.theme.darkMode ? '{{ $partner->is_active ? 'border-amber-600/40 text-amber-400 bg-amber-400/10 hover:bg-amber-400/20' : 'border-emerald-600/40 text-emerald-400 bg-emerald-400/10 hover:bg-emerald-400/20' }}' : '{{ $partner->is_active ? 'border-amber-200 text-amber-600 bg-amber-50 hover:bg-amber-100' : 'border-emerald-200 text-emerald-600 bg-emerald-50 hover:bg-emerald-100' }}'">
                                    @if($partner->is_active)
                                ...
                                <a href="{{ route('superadmin.partners.edit', $partner->id) }}" class="w-8 h-8 rounded-full border flex items-center justify-center transition-colors" :class="$store.theme.darkMode ? 'border-gray-700 text-blue-400 hover:bg-gray-800' : 'border-gray-200 text-blue-600 hover:bg-blue-50'">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                </a>

                                <button @click="isDeleteModalOpen = true; partnerToDelete = {{ $partner->id }}" class="w-8 h-8 rounded-full border flex items-center justify-center transition-colors" :class="$store.theme.darkMode ? 'border-gray-700 text-red-400 hover:bg-gray-800' : 'border-gray-200 text-red-500 hover:bg-red-50'">
        ...
        <div class="mt-8 pt-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t transition-colors duration-300" :class="$store.theme.darkMode ? 'border-gray-800' : 'border-gray-100'">
            ...
            @else
            <a href="{{ $partners->previousPageUrl() }}" class="px-5 py-2.5 rounded-full border text-sm font-bold flex items-center gap-2 transition-all duration-300 hover:bg-gray-50" :class="$store.theme.darkMode ? 'border-gray-700 text-gray-300 hover:bg-gray-800' : 'border-gray-200 text-[#111827]'">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg> Sebelumnya
            </a>
            @endif
            ...
            <div class="flex items-center gap-1.5 font-bold">
                @foreach ($partners->getUrlRange(1, $partners->lastPage()) as $page => $url)
                    ...
                    <button class="w-10 h-10 rounded-full flex items-center justify-center text-sm transition-colors duration-300 bg-[#111827] text-white" :class="$store.theme.darkMode ? 'bg-white text-[#0F172A]' : 'bg-[#111827] text-white'">{{ $page }}</button>
                    @else
                    <a href="{{ $url }}" class="w-10 h-10 rounded-full flex items-center justify-center text-sm transition-colors duration-300 hover:bg-gray-100" :class="$store.theme.darkMode ? 'text-gray-400 hover:bg-gray-800' : 'text-gray-600'">{{ $page }}</a>
                    @endif
                @endforeach
            </div>

            @if ($partners->hasMorePages())
            <a href="{{ $partners->nextPageUrl() }}" class="px-5 py-2.5 rounded-full border text-sm font-bold flex items-center gap-2 transition-all duration-300 hover:bg-gray-50" :class="$store.theme.darkMode ? 'border-gray-700 text-gray-300 hover:bg-gray-800' : 'border-gray-200 text-[#111827]'">
