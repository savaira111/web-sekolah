...
<div class="px-8 pb-8 flex-1 w-full overflow-x-auto">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-4">
            ...
            <div class="flex items-center p-1.5 rounded-full border transition-colors duration-300 shadow-sm" :class="$store.theme.darkMode ? 'bg-[#111827] border-gray-700' : 'bg-white border-gray-200'">
                ...
                <a href="{{ request()->fullUrlWithQuery(['filter' => 'published', 'page' => null]) }}"
                   class="px-5 py-2 rounded-full text-sm font-bold transition-all"
                   :class="$store.theme.darkMode ? '{{ $filter === 'published' ? 'bg-black/20 text-white shadow' : 'text-gray-400 hover:text-white' }}' : '{{ $filter === 'published' ? 'bg-gray-900 text-white shadow' : 'text-gray-600 hover:text-gray-900' }}'">Terbit</a>
                <a href="{{ request()->fullUrlWithQuery(['filter' => 'draft', 'page' => null]) }}"
                   class="px-5 py-2 rounded-full text-sm font-bold transition-all"
                   :class="$store.theme.darkMode ? '{{ $filter === 'draft' ? 'bg-black/20 text-white shadow' : 'text-gray-400 hover:text-white' }}' : '{{ $filter === 'draft' ? 'bg-gray-900 text-white shadow' : 'text-gray-600 hover:text-gray-900' }}'">Draft</a>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="rounded-[2rem] p-6 transition-all duration-300 flex items-center justify-center gap-5 shadow-sm hover:-translate-y-1 hover:shadow-xl" :class="$store.theme.darkMode ? 'bg-[#111827] shadow-black/20' : 'bg-white shadow-gray-200/50'">
            ...
            <div class="flex flex-col">
                ...
                <h3 class="text-3xl font-black leading-none transition-colors duration-300 tracking-tight" :class="$store.theme.darkMode ? 'text-white' : 'text-[#111827]'">{{ number_format($published ?? 0) }}</h3>
            </div>
        </div>
        ...
        <div class="rounded-[2rem] p-6 transition-all duration-300 flex items-center justify-center gap-5 shadow-sm hover:-translate-y-1 hover:shadow-xl" :class="$store.theme.darkMode ? 'bg-[#111827] shadow-black/20' : 'bg-white shadow-gray-200/50'">
            ...
            <div class="flex flex-col">
                ...
                <h3 class="text-3xl font-black leading-none transition-colors duration-300 tracking-tight" :class="$store.theme.darkMode ? 'text-white' : 'text-[#111827]'">{{ number_format($drafts ?? 0) }}</h3>
            </div>
        </div>
        ...
        <div class="rounded-[2rem] p-6 transition-all duration-300 flex items-center justify-center gap-5 shadow-sm hover:-translate-y-1 hover:shadow-xl" :class="$store.theme.darkMode ? 'bg-[#111827] shadow-black/20' : 'bg-white shadow-gray-200/50'">
            ...
            <div class="flex flex-col">
                ...
                <h3 class="text-3xl font-black leading-none transition-colors duration-300 tracking-tight" :class="$store.theme.darkMode ? 'text-white' : 'text-[#111827]'">{{ number_format($views ?? 0) }}</h3>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-[2rem] p-6 lg:p-8 transition-colors duration-300 shadow-sm" :class="$store.theme.darkMode ? 'bg-[#111827] shadow-black/20' : 'bg-white shadow-gray-200/50'">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 pb-6 border-b transition-colors duration-300 gap-4" :class="$store.theme.darkMode ? 'border-gray-800' : 'border-gray-100'">
            <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <div class="flex items-center gap-3" x-data="{ catOpen: false }">
                    {{-- Pencarian --}}
                    <form action="{{ route('superadmin.articles.index') }}" method="GET" class="relative group">
            ...
            <p class="text-[13px] font-bold transition-colors duration-300" :class="$store.theme.darkMode ? 'text-gray-400' : 'text-gray-400'">Menampilkan {{ $articles->firstItem() ?? 0 }}-{{ $articles->lastItem() ?? 0 }} dari {{ number_format($articles->total() ?? 0) }} artikel</p>
        </div>
                    ...
                    <tr class="border-b last:border-0 transition-colors group" :class="$store.theme.darkMode ? 'border-gray-800 hover:bg-black/20/50' : 'border-gray-50 hover:bg-gray-50/50'">
                        <td class="py-5 transition-colors duration-300 pl-2">
                            <div class="flex items-center gap-4">
                                ...
                                @endif
                                <div>
                                    <a href="{{ route('news.detail', $article->slug) }}" target="_blank" class="font-bold text-[15.5px] hover:text-blue-600 transition-colors duration-300 block mb-0.5 truncate max-w-[350px]" :class="$store.theme.darkMode ? 'text-white' : 'text-[#111827]'" title="{{ $article->title }}">{{ $article->title }}</a>
                                    <p class="text-[13px] transition-colors duration-300 font-medium" :class="$store.theme.darkMode ? 'text-gray-400' : 'text-gray-500'">Diperbarui {{ $article->updated_at->diffForHumans() }} • {{ number_format($article->views_count ?? 0) }} kali dilihat</p>
                                </div>
                            </div>
                        </td>
                        <td class="py-5">
                            <span class="inline-block px-3 py-1 font-black text-[10px] rounded-full uppercase tracking-widest transition-colors duration-300" :class="$store.theme.darkMode ? 'bg-blue-500/10 text-blue-400' : 'bg-[#EBF1FF] text-[#2563EB]'">{{ $article->category }}</span>
                        </td>
                        <td class="py-5">
                            <div class="flex items-center gap-2.5">
                                ...
                                <span class="text-[13px] font-semibold whitespace-nowrap" :class="$store.theme.darkMode ? 'text-gray-200' : 'text-gray-700'">{{ $article->author->name ?? 'Admin' }}</span>
                            </div>
                        </td>
                        <td class="py-5">
                        ...
                        <td class="py-5 text-right pr-2">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <a href="{{ route('superadmin.articles.edit', $article->id) }}" class="w-8 h-8 rounded-full border flex items-center justify-center transition-colors" :class="$store.theme.darkMode ? 'border-gray-700 text-blue-400 hover:bg-gray-800' : 'border-gray-200 text-blue-600 hover:bg-blue-50'">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                </a>

                                <button @click="isDeleteModalOpen = true; articleToDelete = {{ $article->id }}" class="w-8 h-8 rounded-full border flex items-center justify-center transition-colors" :class="$store.theme.darkMode ? 'border-gray-700 text-red-400 hover:bg-gray-800' : 'border-gray-200 text-red-500 hover:bg-red-50'">
        ...
        <div class="mt-8 pt-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t transition-colors duration-300" :class="$store.theme.darkMode ? 'border-gray-800' : 'border-gray-100'">
            ...
            @else
            <a href="{{ $articles->previousPageUrl() }}" class="px-5 py-2.5 rounded-full border text-sm font-bold flex items-center gap-2 transition-all duration-300 hover:bg-gray-50" :class="$store.theme.darkMode ? 'border-gray-700 text-gray-300 hover:bg-gray-800' : 'border-gray-200 text-[#111827]'">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg> Sebelumnya
            </a>
            @endif
            ...
            <div class="flex items-center gap-1.5 font-bold">
                ...
                @if($start > 1)
                    <a href="{{ $articles->url(1) }}" class="w-10 h-10 rounded-full flex items-center justify-center text-sm transition-colors duration-300 hover:bg-gray-100" :class="$store.theme.darkMode ? 'text-gray-400 hover:bg-gray-800' : 'text-gray-600'">1</a>
                    @if($start > 2)
                ...
                @if($end < $articles->lastPage())
                    @if($end < $articles->lastPage() - 1)
                        <span class="w-10 h-10 flex items-center justify-center" :class="$store.theme.darkMode ? 'text-gray-600' : 'text-gray-400'">...</span>
                    @endif
                    <a href="{{ $articles->url($articles->lastPage()) }}" class="w-10 h-10 rounded-full flex items-center justify-center text-sm transition-colors duration-300 hover:bg-gray-100" :class="$store.theme.darkMode ? 'text-gray-400 hover:bg-gray-800' : 'text-gray-600'">{{ $articles->lastPage() }}</a>
                @endif
            </div>

            @if ($articles->hasMorePages())
            <a href="{{ $articles->nextPageUrl() }}" class="px-5 py-2.5 rounded-full border text-sm font-bold flex items-center gap-2 transition-all duration-300 hover:bg-gray-50" :class="$store.theme.darkMode ? 'border-gray-700 text-gray-300 hover:bg-gray-800' : 'border-gray-200 text-[#111827]'">
