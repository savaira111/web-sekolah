<aside class="fixed md:static inset-y-0 left-0 sidebar h-full flex flex-col z-50 shrink-0 border-r border-gray-800 transform overflow-hidden" 
    ...
    <div class="h-20 flex items-center px-6 mb-0">
        <div class="w-11 h-11 bg-white/50 dark:bg-white/10 backdrop-blur-md rounded-xl flex items-center justify-center shrink-0 overflow-hidden border border-black/5 dark:border-white/20 shadow-sm dark:shadow-xl">
            <img src="{{ asset('images/logo-smk.png') }}" alt="Logo" class="w-7 h-7 object-contain">
        </div>
    ...
    <div class="flex-1 overflow-y-auto pt-2 pb-6 px-4 space-y-1">
        ...
        {{ request()->routeIs('superadmin.dashboard') ? 'sidebar-active' : 'sidebar-item' }}">
        ...
        class="flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium text-sm transition-all
        {{ request()->routeIs('superadmin.albums.*') ? 'sidebar-active' : 'sidebar-item' }}">
        ...
        class="flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium text-sm transition-all
        {{ request()->routeIs('superadmin.landing-stats.*') ? 'sidebar-active' : 'sidebar-item' }}">
        ...
        class="flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium text-sm transition-all
        {{ request()->routeIs('superadmin.partners.*') ? 'sidebar-active' : 'sidebar-item' }}">
        ...
        class="flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium text-sm transition-all
        {{ request()->routeIs('superadmin.users.*') ? 'sidebar-active' : 'sidebar-item' }}">
        ...
        <div x-data="{ open: {{ request()->routeIs('superadmin.articles.*') || request()->routeIs('superadmin.comments.*') ? 'true' : 'false' }} }">
            ...
            {{ request()->routeIs('superadmin.articles.*') || request()->routeIs('superadmin.comments.*') ? 'sidebar-active' : 'sidebar-item' }}">
            ...
            <div x-show="open" 
                ...
                <a href="/superadmin/articles" 
                   class="block py-2 text-sm transition-colors {{ request()->routeIs('superadmin.articles.*') ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-500 hover:text-blue-600 dark:hover:text-white' }}">
                    Berita Management
                </a>
                <a href="/superadmin/comments" 
                   class="flex items-center justify-between py-2 text-sm transition-colors {{ request()->routeIs('superadmin.comments.*') ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-500 hover:text-blue-600 dark:hover:text-white' }}">
                    <span>Manajemen Komentar</span>
                    @if($pendingCommentsCount > 0)
                        <span class="px-2 py-0.5 bg-blue-500/10 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 border border-blue-500/20 dark:border-blue-500/30 text-[10px] font-bold rounded-full mr-2">{{ $pendingCommentsCount }}</span>
                    @endif
                </a>
            </div>
        </div>
        ...
        class="flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium text-sm transition-all
        {{ request()->routeIs('superadmin.messages.*') ? 'sidebar-active' : 'sidebar-item' }}">
            ...
            <div class="flex items-center justify-between flex-1 truncate">
                ...
                @if($unreadMessagesCount > 0)
                    <span class="px-2 py-0.5 bg-amber-500/20 text-amber-500 border border-amber-500/30 text-[10px] font-bold rounded-full">{{ $unreadMessagesCount }}</span>
                @endif
            </div>
        </a>
        @endif
        ...
        class="flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium text-sm transition-all
        {{ request()->routeIs('superadmin.ppdb.*') ? 'sidebar-active' : 'sidebar-item' }}">
            ...
            <div class="flex items-center justify-between flex-1 truncate">
                ...
                @if($pendingPpdbCount > 0)
                    <span class="px-2 py-0.5 bg-blue-500/20 text-blue-400 border border-blue-500/30 text-[10px] font-bold rounded-full">{{ $pendingPpdbCount }}</span>
                @endif
            </div>
        </a>
        @endif
        ...
        <div x-data="{ open: {{ request()->routeIs('superadmin.extracurricular-registrations.*') || request()->routeIs('superadmin.eskul.*') ? 'true' : 'false' }} }">
            <button @click="open = !open"
            class="flex items-center justify-between w-full px-4 py-3.5 rounded-xl font-medium text-sm transition-all
            {{ request()->routeIs('superadmin.extracurricular-registrations.*') || request()->routeIs('superadmin.eskul.*') ? 'sidebar-active' : 'sidebar-item' }}">
                <div class="flex items-center gap-3">
                    <div class="flex items-center justify-center w-5 h-5">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" class="w-full h-full">
            ...
            <div x-show="open" 
                ...
                <a href="{{ route('superadmin.eskul.index') }}" 
                   class="block py-2 text-sm transition-colors {{ request()->routeIs('superadmin.eskul.index') ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-500 hover:text-blue-600 dark:hover:text-white' }}">
                    Manajemen Eskul
                </a>
                <a href="{{ route('superadmin.extracurricular-registrations.index') }}" 
                   class="flex items-center justify-between py-2 text-sm transition-colors {{ request()->routeIs('superadmin.extracurricular-registrations.index') ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-500 hover:text-blue-600 dark:hover:text-white' }}">
                    <span>Pendaftaran</span>
                    @if($pendingEskulCount > 0)
                        <span class="px-2 py-0.5 bg-purple-500/10 dark:bg-purple-500/20 text-purple-600 dark:text-purple-400 border border-purple-500/20 dark:border-purple-500/30 text-[10px] font-bold rounded-full mr-2">{{ $pendingEskulCount }}</span>
                    @endif
                </a>
            </div>
        </div>
