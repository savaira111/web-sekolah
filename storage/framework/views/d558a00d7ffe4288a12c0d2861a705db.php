<?php $__env->startSection('title', 'Manajemen Komentar - Admin Mahput'); ?>
                ...
                <div class="rounded-xl shadow-sm border p-6 hover:shadow-md transition-shadow transition-colors duration-300" :class="$store.theme.darkMode ? 'bg-[#111827] border-gray-700' : 'bg-white border-gray-200'">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold transition-colors duration-300" :class="$store.theme.darkMode ? 'text-white' : 'text-gray-900'"><?php echo e($totalComments ?? '154'); ?></h3>
                ...
                <div class="rounded-xl shadow-sm border p-6 hover:shadow-md transition-shadow transition-colors duration-300" :class="$store.theme.darkMode ? 'bg-[#111827] border-gray-700' : 'bg-white border-gray-200'">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold transition-colors duration-300" :class="$store.theme.darkMode ? 'text-white' : 'text-gray-900'"><?php echo e($pendingComments ?? '12'); ?></h3>
            ...
            <div class="rounded-xl shadow-sm border overflow-hidden transition-colors duration-300" :class="$store.theme.darkMode ? 'bg-[#111827] border-gray-700' : 'bg-white border-gray-200'">
                    ...
                    <table class="w-full text-left border-collapse font-medium">
                        ...
                        <tbody class="divide-y transition-colors duration-300" :class="$store.theme.darkMode ? 'divide-gray-700' : 'divide-gray-200'">
                            ...
                            <tr class="transition-colors duration-300" :class="$store.theme.darkMode ? 'hover:bg-black/20' : 'hover:bg-gray-50'">
                                <td class="px-8 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-sm shrink-0 uppercase">
                                            <?php echo e(substr($comment->name, 0, 1)); ?>

                                        </div>
                                        <div class="max-w-[150px]">
                                            <p class="font-bold transition-colors duration-300 truncate" :class="$store.theme.darkMode ? 'text-white' : 'text-gray-900'"><?php echo e($comment->name); ?></p>
                                            <p class="text-[11px] mt-0.5 transition-colors duration-300 truncate" :class="$store.theme.darkMode ? 'text-gray-400' : 'text-gray-500'"><?php echo e($comment->email); ?></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-4">
                                ...
                                <td class="px-8 py-4">
                                    <a href="<?php echo e(route('news.detail', $comment->article->slug)); ?>" target="_blank" class="text-xs font-bold text-blue-600 hover:text-blue-700 line-clamp-1 max-w-[180px]">
                                ...
                                <td class="px-8 py-4 text-right whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-3">
                                        <?php if($comment->status === 'pending'): ?>
                                        <button type="button" @click="isApproveModalOpen = true; commentToApprove = <?php echo e($comment->id); ?>" class="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-600 hover:text-white transition-all shadow-sm" title="Approve">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                        </button>
                                        <?php endif; ?>
                                        
                                        <?php if($comment->status !== 'rejected'): ?>
                                        <button type="button" @click="isRejectModalOpen = true; commentToReject = <?php echo e($comment->id); ?>" class="p-2 bg-orange-100 text-orange-600 rounded-lg hover:bg-orange-600 hover:text-white transition-all shadow-sm" title="Reject / Cancel">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                        </button>
                                        <?php endif; ?>
                                        <button type="button" @click="isDeleteModalOpen = true; commentToDelete = <?php echo e($comment->id); ?>" class="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-600 hover:text-white transition-all shadow-sm" title="Hapus">
                ...
                <div class="px-8 py-6 border-t flex items-center justify-between transition-colors duration-300" :class="$store.theme.darkMode ? 'border-gray-700' : 'border-gray-200'">
                    <p class="text-sm transition-colors duration-300" :class="$store.theme.darkMode ? 'text-gray-400' : 'text-gray-600'">
                        Menampilkan <?php echo e($comments->firstItem()); ?> ke <?php echo e($comments->lastItem()); ?> dari <?php echo e($comments->total()); ?> data
                    </p>
...
<?php echo $__env->make('layouts.superadmin.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\web-sekolah\resources\views/superadmin/comments/index.blade.php ENDPATH**/ ?>