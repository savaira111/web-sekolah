<?php $__env->startSection('title', $partner->name . ' - Mitra Industri SMKS Mahaputra Cerdas Utama'); ?>
            ...
            <div class="grid lg:grid-cols-2">
                ...
                <div class="p-8 lg:p-10 space-y-6">
                    <div class="flex items-center gap-4">
                        <div class="w-16 h-16 rounded-xl bg-gray-50 p-3 border border-gray-100 flex items-center justify-center overflow-hidden shrink-0 shadow-inner">
                            <?php if($partner->logo): ?>
                                <img src="<?php echo e(asset('storage/' . $partner->logo)); ?>" alt="<?php echo e($partner->name); ?>" class="w-full h-full object-contain">
                            <?php else: ?>
                        ...
                        <div>
                            ...
                            <h1 class="text-3xl lg:text-4xl font-black text-[#0F172A] mt-1 tracking-tight"><?php echo e($partner->name); ?></h1>
                        </div>
                    </div>

                    <p class="text-base text-gray-500 leading-relaxed font-medium">
                        <?php echo e($partner->description ?? 'Deskripsi perusahaan mitra belum tersedia.'); ?>
                    ...
                    <div class="flex flex-wrap gap-3 pt-2">
                        ...
                        <a href="<?php echo e($partner->website_url); ?>" target="_blank" class="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 text-white text-xs font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all">
                ...
                <div class="relative min-h-[300px] h-full overflow-hidden">
                    <?php if($partner->featured_image): ?>
                        <img src="<?php echo e(asset('storage/' . $partner->featured_image)); ?>" alt="<?php echo e($partner->name); ?>" class="absolute inset-0 w-full h-full object-cover">
                    <?php else: ?>
...
<section class="py-14 bg-white">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 h-auto md:h-[450px]">
            <?php if($partner->gallery_images && count($partner->gallery_images) > 0): ?>
                <!-- Real Gallery Images -->
                <?php $galleryCount = count($partner->gallery_images); ?>
                
                <!-- Main Large Image (First) -->
                <div class="md:col-span-2 relative group overflow-hidden rounded-[2.5rem] shadow-xl min-h-[300px]">
                     <img src="<?php echo e(asset('storage/' . $partner->gallery_images[0])); ?>" alt="Gallery 1" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                     <div class="absolute inset-0 bg-gradient-to-t from-black/20 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                </div>

                <!-- Right Side Grid -->
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <?php if(isset($partner->gallery_images[1])): ?>
                    <div class="relative group overflow-hidden rounded-[2.5rem] shadow-lg md:col-span-2 h-[217px]">
                        <img src="<?php echo e(asset('storage/' . $partner->gallery_images[1])); ?>" alt="Gallery 2" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                    </div>
                    <?php endif; ?>
                    
                    <?php if(isset($partner->gallery_images[2])): ?>
                    <div class="relative group overflow-hidden rounded-[2.5rem] shadow-lg h-[217px]">
                        <img src="<?php echo e(asset('storage/' . $partner->gallery_images[2])); ?>" alt="Gallery 3" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                    </div>
                    <?php endif; ?>

                    <?php if(isset($partner->gallery_images[3])): ?>
                    <div class="relative group overflow-hidden rounded-[2.5rem] shadow-lg h-[217px]">
                        <img src="<?php echo e(asset('storage/' . $partner->gallery_images[3])); ?>" alt="Gallery 4" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                    </div>
                    <?php endif; ?>
                </div>
            <?php else: ?>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 relative text-center">
        ...
        <div class="relative z-10 space-y-8">
            ...
            <div class="flex flex-col items-center">
                ...
                <p class="text-sm font-bold text-blue-600 uppercase tracking-widest mt-1"><?php echo e($partner->name); ?></p>
            </div>
        </div>
    </div>
</section>
...
<section class="pt-16 pb-12 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-16">
            <div>
                <h2 class="text-3xl font-black text-[#0F172A] tracking-tight">Siswa PKL di <?php echo e($partner->name); ?></h2>
                ...
                <div class="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm hover:shadow-xl transition-all group border-t-4 border-t-blue-600">
                    <div class="flex items-center gap-4 mb-6">
                        <div class="w-14 h-14 rounded-2xl overflow-hidden shadow-md">
                            <img src="<?php echo e(!empty($t['photo_url']) ? asset('storage/' . $t['photo_url']) : 'https://ui-avatars.com/api/?name='.urlencode($t['author'] ?? 'User').'&background=3B82F6&color=fff'); ?>" alt="Siswa PKL" class="w-full h-full object-cover">
                        </div>
                        <div>
                            <h4 class="font-black text-[#0F172A]"><?php echo e($t['author'] ?? 'Anonim'); ?></h4>
                            <p class="text-[10px] font-bold text-blue-600 uppercase tracking-wider"><?php echo e($t['role'] ?? 'Siswa'); ?></p>
                            <div class="flex gap-1 mt-1">
                                <?php $rating = intval($t['rating'] ?? 5); ?>
                                <?php for($i = 1; $i <= 5; $i++): ?>
                                    <svg class="w-3.5 h-3.5 <?php echo e($i <= $rating ? 'text-amber-400 drop-shadow-sm' : 'text-gray-200'); ?>" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                                <?php endfor; ?>
                            </div>
                        </div>
                    </div>
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center gap-3 text-xs font-bold text-gray-400 pt-2 border-t border-gray-50">
                            ...
                            Penempatan: <?php echo e($partner->location ?? 'Head Office'); ?>
        ...
        <div class="text-center mt-12">
            <a href="#" class="text-[10px] font-black text-blue-600 hover:text-blue-800 transition-colors uppercase tracking-[0.2em] bg-blue-50 px-6 py-3 rounded-full border border-blue-100">
                Lihat Semua Siswa PKL di <?php echo e($partner->name); ?>
        ...
        <div class="relative bg-gradient-to-br from-[#0256B3] to-[#014188] rounded-[2.5rem] p-10 lg:p-12 overflow-hidden shadow-2xl text-center">
            <div class="relative z-10 max-w-2xl mx-auto space-y-6">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
                    <a href="<?php echo e(route('contact')); ?>" class="w-full sm:w-auto px-8 py-4 bg-white text-blue-700 rounded-xl font-black text-sm shadow-xl shadow-blue-900/20 hover:bg-blue-50 transition-all active:scale-95 uppercase tracking-wide">
                        Daftar Jadi Mitra
                    </a>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\web-sekolah\resources\views/mitra_detail.blade.php ENDPATH**/ ?>