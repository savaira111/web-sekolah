<?php $__env->startSection('title', 'Edit Ekstrakurikuler - Admin Mahput'); ?>
...
<div x-data="editEskulForm()" class="flex-1 flex flex-col min-h-screen transition-colors duration-300" :class="$store.theme.darkMode ? 'bg-black/20' : 'bg-[#F8FAFC]'">
    ...
    <div class="px-8 pb-12">
        ...
        <div class="flex items-center gap-2 mt-8 mb-4 text-xs font-bold text-gray-400">
            ...
            <a href="<?php echo e(route('superadmin.eskul.index')); ?>">Ekstrakurikuler</a>
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span :class="$store.theme.darkMode ? 'text-white' : 'text-gray-900'">Edit <?php echo e($eskul->name ?? 'Pramuka'); ?></span>
        </div>
        ...
        <form x-ref="form" action="<?php echo e(route('superadmin.eskul.update', $eskul)); ?>" method="POST" enctype="multipart/form-data">
            <?php echo csrf_field(); ?>
                    ...
                    <div class="bg-white rounded-[2rem] p-8 shadow-sm border transition-colors duration-300" :class="$store.theme.darkMode ? 'bg-[#111827] border-gray-800' : 'bg-white border-gray-100'">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="name" value="<?php echo e($eskul->name); ?>" required class="w-full px-5 py-4 rounded-xl border-none font-bold text-gray-700 transition-all outline-none" :class="$store.theme.darkMode ? 'bg-black/20 text-white' : 'bg-gray-50/80 text-gray-700'">
                            </div>
                                    ...
                                    <div class="relative">
                                        <select name="category" required class="w-full px-5 py-4 rounded-xl border-none font-bold text-gray-700 transition-all outline-none appearance-none" :class="$store.theme.darkMode ? 'bg-black/20 text-white' : 'bg-gray-50/80 text-gray-700'">
                                            <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $id => $name): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <option value="<?php echo e($id); ?>" <?php echo e($eskul->category == $id ? 'selected' : ''); ?>><?php echo e($name); ?></option>
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="5" required class="w-full px-5 py-4 rounded-2xl border-none font-bold text-gray-700 transition-all outline-none leading-relaxed" :class="$store.theme.darkMode ? 'bg-black/20 text-white' : 'bg-gray-50/80 text-gray-700'"><?php echo e($eskul->description); ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-[2rem] p-8 shadow-sm border transition-colors duration-300" :class="$store.theme.darkMode ? 'bg-[#111827] border-gray-800' : 'bg-white border-gray-100'">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <input type="text" name="schedule_info" value="<?php echo e($eskul->schedule_info); ?>" placeholder="Contoh: Jumat" class="w-full px-5 py-4 rounded-xl border-none font-bold text-gray-700 transition-all outline-none" :class="$store.theme.darkMode ? 'bg-black/20 text-white' : 'bg-gray-50/80 text-gray-700'">
                            </div>
                            <div>
                ...
                <div class="w-full lg:w-[380px] space-y-8">
                    ...
                    <div class="bg-white rounded-[2rem] p-8 shadow-sm border transition-colors duration-300" :class="$store.theme.darkMode ? 'bg-[#111827] border-gray-800' : 'bg-white border-gray-100'">
                        ...
                        <div class="relative aspect-video rounded-2xl overflow-hidden border-2 border-dashed transition-all" :class="$store.theme.darkMode ? 'border-gray-800 bg-black/20' : 'border-gray-100 bg-gray-50/50'">
                            <img :src="imagePreview || '<?php echo e($eskul->image ? asset('storage/' . $eskul->image) : asset('images/gedung-sekolah.jpg')); ?>'" class="w-full h-full object-cover">
                    ...
                    <div class="space-y-4">
                        ...
                        <a href="<?php echo e(route('superadmin.eskul.index')); ?>" class="w-full py-5 bg-[#E2E8F0] text-gray-600 font-black rounded-2xl flex items-center justify-center gap-3 hover:bg-gray-300 transition-all" :class="$store.theme.darkMode ? 'bg-gray-800 text-gray-400 hover:bg-gray-700' : ''">
...
<script>
    function editEskulForm() {
        return {
            imagePreview: null,
            isActive: <?php echo e($eskul->is_active ? 'true' : 'false'); ?>,
            handleImage(e) {
            ...
            async submitForm() {
                ...
                try {
                    const response = await fetch("<?php echo e(route('superadmin.eskul.update', $eskul)); ?>", {
                        method: 'POST', // Laravel will spoof PUT due to <?php echo method_field('PUT'); ?>
                        body: formData,
                        headers: {
                    ...
                    if (response.ok) {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || "<?php echo e(route('superadmin.eskul.index')); ?>";
                        });
                    } else {
...
<?php echo $__env->make('layouts.superadmin.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\web-sekolah\resources\views/superadmin/eskul/edit.blade.php ENDPATH**/ ?>